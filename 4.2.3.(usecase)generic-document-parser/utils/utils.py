...
from models.generic import OCROutput 
...
def save_results_to_json(filenames: str, results: list[OCROutput], save_dir: Path) -> None:
    for filename, result in zip(filenames, results):
        save_path = Path(save_dir, f"{filename.stem}.json")
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump([r.model_dump_json() for r in result], f, indent=4, ensure_ascii=True)
